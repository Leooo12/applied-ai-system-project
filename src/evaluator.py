...
def _average_confidence(results: List[CaseResult]) -> Optional[float]:
    """Mean of every case's parsed `confidence=...`, when the field is present."""
    values = []
    for r in results:
        if r.parser_result.startswith("confidence="):
            values.append(float(r.parser_result.split("=", 1)[1]))
    if not values:
        return None
...
def _write_markdown(results: List[CaseResult], path: str, passed: int, total: int) -> None:
    ...
    avg_confidence = _average_confidence(results)
    avg_confidence_line = (
        f"{avg_confidence:.2f}" if avg_confidence is not None else "n/a (no cases reported a confidence value)"
    )
    lines = [
        ...
        f"- Average parser confidence: {avg_confidence_line}",
        "",
